...
def test_embed_query_uses_query_prefix(
    ...
    # calls[0] is load()'s own internal dim-validation probe
...
def test_validate_dim_raises_and_logs_on_mismatch(
    ...
    engine = embeddings.EmbeddingEngine()
    with (
        caplog.at_level(logging.ERROR, logger='study_mcp.core.embeddings'),
        pytest.raises(RuntimeError, match='EMBEDDING_DIM mismatch'),
    ):
        engine.load()
